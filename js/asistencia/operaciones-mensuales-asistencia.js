  ...
  panel.innerHTML = `
    ...
    <div class="operaciones-grid">
      <article class="operacion-tarjeta">
        ...
        <div class="seleccion-masiva"><label><input id="seleccionarTodasHorasExtra" type="checkbox"> Seleccionar todos</label><span id="resumenSeleccionHorasExtra">0 seleccionados</span></div>
...
function renderizarCandidatos() {
  ...
  const candidatos = reporte.registros.filter((r) => Number(r.minutosExtraGenerados) > 0);
  const lista = panel.querySelector("#listaHorasExtraMasiva");
  lista.innerHTML = candidatos.length ? candidatos.map((r) => `
    <label class="fila-extra-masiva">
      <input type="checkbox" data-colaborador-id="${escapar(r.colaboradorId)}" data-nombre="${escapar(r.nombre)}" data-minutos="${Number(r.minutosExtraGenerados) || 0}">
    ...
    </label>`).join("") : '<p class="sin-operaciones">No existen horas extra generadas en este período.</p>';
  actualizarSeleccion();
}

function seleccionarTodos(evento) {
  panel.querySelectorAll('#listaHorasExtraMasiva input[type="checkbox"]').forEach((input) => input.checked = evento.target.checked);
  actualizarSeleccion();
}

function actualizarSeleccion() {
  const total = panel.querySelectorAll('#listaHorasExtraMasiva input[type="checkbox"]:checked').length;
  panel.querySelector("#resumenSeleccionHorasExtra").textContent = `${total} seleccionados`;
}
...
async function aplicarDecisionMasiva() {
  ...
  const seleccionados = [...panel.querySelectorAll('#listaHorasExtraMasiva input[type="checkbox"]:checked')];
  ...
  seleccionados.forEach((input) => {
    const totalPeriodo = Number(input.dataset.minutos) || 0;
    const diasConExtra = reporte.registros.find((r) => r.colaboradorId === input.dataset.colaboradorId)?.detalles.filter((d) => Number(d.extraGenerada) > 0) || [];
    diasConExtra.forEach((detalle) => {
      ...
      const id = [empresaId, input.dataset.colaboradorId, detalle.fecha].map(limpiarId).join("_");
      batch.set(doc(db, "aprobacionesHorasExtra", id), {
        empresaId, colaboradorId: input.dataset.colaboradorId, colaboradorNombre: input.dataset.nombre,

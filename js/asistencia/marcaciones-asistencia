...
export function iniciarMarcacionesAsistencia(){
    ...
    cuerpoMarcaciones =
        document.getElementById("cuerpoMarcacionesAsistencia");

    buscarMarcaciones =
        document.getElementById("buscarMarcacionesAsistencia");

    informacionMarcaciones =
        document.getElementById("informacionMarcacionesAsistencia");

    fechaActualMarcaciones =
        document.getElementById("fechaActualMarcaciones");
    ...
    buscarMarcaciones?.addEventListener(
        "input",
        renderizarMarcaciones
    );


    document.getElementById("btnDiaAnteriorMarcaciones")
    ?.addEventListener("click", ()=>{
        document.getElementById("btnDiaAnteriorAsistencia")?.click();
    });


    document.getElementById("btnDiaSiguienteMarcaciones")
    ?.addEventListener("click", ()=>{
        document.getElementById("btnDiaSiguienteAsistencia")?.click();
    });


    document.getElementById("btnHoyMarcaciones")
    ?.addEventListener("click", ()=>{
...
function prepararMarcaciones(
    fecha,
    colaboradores,
    marcaciones
){

    const colaboradoresPorId =
        new Map(
    ...
    .map(marcacion=>{
        ...
        const colaborador =
            colaboradoresPorId.get(marcacion.colaboradorId) || {};
...
function renderizarMarcaciones(){
    ...
    cuerpoMarcaciones.innerHTML = filtradas.map(marcacion=>{
...
function actualizarInformacion(total){
    if(informacionMarcaciones){
        informacionMarcaciones.textContent =
            `Mostrando ${total} ${total === 1 ? "marcación" : "marcaciones"}`;
    }
}
...
function actualizarFechaVisible(fecha){
    ...
    fechaActualMarcaciones.textContent =

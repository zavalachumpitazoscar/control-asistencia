...
function renderizar() {
  $("listaRelojesBiometricos").innerHTML = relojes.length ? relojes.map((reloj) => {const historial=(reloj.historialComandos||[]).slice(-5).reverse();return `<article class="biometric-row"><div><b>${escapar(reloj.nombre || "Reloj ZKTeco")}</b><small>${escapar(reloj.modelo || "Modelo no indicado")} · Serie ${escapar(reloj.id)} · Firmware ${escapar(reloj.firmware||"pendiente")}</small><span>${escapar(nombreSucursal(reloj.sucursalId))} · Última conexión: ${escapar(fechaVisible(reloj.ultimaConexionEn))} · Última marcación: ${escapar(fechaVisible(reloj.ultimaMarcacionEn))}</span><em class="biometric-state ${relojConectado(reloj) ? "" : "inactive"}">${relojConectado(reloj) ? "CONECTADO" : "DESCONECTADO"}</em><em class="biometric-state ${reloj.estado === "ACTIVO" ? "" : "inactive"}">${escapar(reloj.estado || "INACTIVO")}</em><details><summary>Historial de comandos (${reloj.historialComandos?.length||0})</summary>${historial.length?historial.map(x=>`<small>${escapar(x.descripcion||"Comando")} · ${escapar(x.estado||"PROGRAMADO")} · ${escapar(fechaVisible(x.enviadoEn||x.fecha))}</small>`).join(""):"<small>Sin comandos registrados.</small>"}</details></div><div><button data-editar-reloj="${escapar(reloj.id)}">Editar nombre</button><button data-cambiar-reloj="${escapar(reloj.id)}">${reloj.estado === "ACTIVO" ? "Desactivar" : "Activar"}</button></div></article>`;}).join("") : '<div class="usage-empty">Todavía no hay relojes autorizados.</div>';
...
$("listaRelojesBiometricos")?.addEventListener("click", async (evento) => {
  const editar = evento.target.closest("[data-editar-reloj]");
  if (editar) { const reloj=relojes.find(item=>item.id===editar.dataset.editarReloj);if(!reloj)return;const respuesta=await Swal.fire({title:"Nombre del reloj",input:"text",inputValue:reloj.nombre||"",inputLabel:`Serie: ${reloj.id}`,showCancelButton:true,confirmButtonText:"Guardar",inputValidator:v=>!String(v||"").trim()?"Ingresa un nombre.":undefined});if(respuesta.isConfirmed){await updateDoc(doc(db,"relojesBiometricos",reloj.id),{nombre:String(respuesta.value).trim().slice(0,80),actualizadoEn:serverTimestamp(),actualizadoPor:auth.currentUser?.uid||""});await auditar("EDITAR_NOMBRE_RELOJ",{empresaId:empresaId(),relojSerial:reloj.id,nombre:respuesta.value});await cargar();}return; }

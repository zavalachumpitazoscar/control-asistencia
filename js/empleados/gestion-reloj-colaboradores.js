...
async function revisarEImportar(usuarios, colaboradores, empresaId, reloj) {
  ...
  const html = `<div class="zk-import-summary"><p><b>${filas.length}</b> usuarios recibidos · <b>${filas.length - nuevos.length}</b> ya existen · <b>${nuevos.length}</b> por revisar</p>${filas.map((fila) => fila.existente ? `<div class="zk-import-row existing"><b>PIN ${escapar(fila.usuario.pin)}</b><span>Ya corresponde a ${escapar(nombre(fila.existente))} · ${escapar(documento(fila.existente))}</span></div>` : `<div class="zk-import-row"><label><input type="checkbox" data-zk-seleccionar="${fila.indice}" checked> Importar PIN ${escapar(fila.usuario.pin)}</label><div><input data-zk-dni="${fila.indice}" maxlength="8" inputmode="numeric" value="${escapar(fila.sugerido)}" placeholder="DNI de 8 dígitos"><input data-zk-nombres="${fila.indice}" value="${escapar(fila.nombres)}" placeholder="Nombres"><input data-zk-apellidos="${fila.indice}" value="${escapar(fila.apellidos)}" placeholder="Apellidos"></div>${fila.pin.length !== 8 ? `<small>El PIN no contiene un DNI completo. Revisa y confirma el documento antes de importar.</small>` : ""}</div>`).join("")}</div>`;
  const respuesta = await Swal.fire({ title:"Revisar usuarios del reloj", html, width:900, showCancelButton:true, confirmButtonText:"Importar seleccionados", cancelButtonText:"Cancelar", focusConfirm:false, preConfirm:() => {
